...
import jdk.incubator.vector.ByteVector.SPECIES_128
...
internal class StructuralIndexer(private val bitIndexes: BitIndexes) {
    ...
    private fun index128(buffer: ByteArray, length: Int) {
        var prevInString = 0L
        var prevEscaped = 0L
        var prevStructurals = 0L
        var unescapedCharsError = 0L
        ...
        // Using SPECIES_512 here is not a mistake. Each iteration of the below loop processes four 128-bit chunks,
        // so effectively it processes 512 bits at once.
        val loopBound = SPECIES_512.loopBound(length)
        var offset = 0
        var blockIndex = 0
        while (offset < loopBound) {
            val chunk0 = ByteVector.fromArray(SPECIES_128, buffer, offset)
            val chunk1 = ByteVector.fromArray(SPECIES_128, buffer, offset + 16)
            val chunk2 = ByteVector.fromArray(SPECIES_128, buffer, offset + 32)
            val chunk3 = ByteVector.fromArray(SPECIES_128, buffer, offset + 48)

            // string scanning
            val backslash0 = chunk0.eq(BACKSLASH).toLong()
            val backslash1 = chunk1.eq(BACKSLASH).toLong()
            val backslash2 = chunk2.eq(BACKSLASH).toLong()
            val backslash3 = chunk3.eq(BACKSLASH).toLong()
            var backslash = backslash0 or (backslash1 shl 16) or (backslash2 shl 32) or (backslash3 shl 48)
            ...
            val unescaped0 = chunk0.compare(ULE, LAST_CONTROL_CHARACTER).toLong()
            val unescaped1 = chunk1.compare(ULE, LAST_CONTROL_CHARACTER).toLong()
            val unescaped2 = chunk2.compare(ULE, LAST_CONTROL_CHARACTER).toLong()
            val unescaped3 = chunk3.compare(ULE, LAST_CONTROL_CHARACTER).toLong()
            val unescaped = unescaped0 or (unescaped1 shl 16) or (unescaped2 shl 32) or (unescaped3 shl 48)

            val quote0 = chunk0.eq(QUOTE).toLong()
            val quote1 = chunk1.eq(QUOTE).toLong()
            val quote2 = chunk2.eq(QUOTE).toLong()
            val quote3 = chunk3.eq(QUOTE).toLong()
            val quote = (quote0 or (quote1 shl 16) or (quote2 shl 32) or (quote3 shl 48)) and escaped.inv()
            ...
            val chunk0Low: VectorShuffle<Byte> = chunk0.and(LOW_NIBBLE_MASK).toShuffle()
            val chunk1Low: VectorShuffle<Byte> = chunk1.and(LOW_NIBBLE_MASK).toShuffle()
            val chunk2Low: VectorShuffle<Byte> = chunk2.and(LOW_NIBBLE_MASK).toShuffle()
            val chunk3Low: VectorShuffle<Byte> = chunk3.and(LOW_NIBBLE_MASK).toShuffle()

            val whitespace0 = chunk0.eq(WHITESPACE_TABLE.rearrange(chunk0Low)).toLong()
            val whitespace1 = chunk1.eq(WHITESPACE_TABLE.rearrange(chunk1Low)).toLong()
            val whitespace2 = chunk2.eq(WHITESPACE_TABLE.rearrange(chunk2Low)).toLong()
            val whitespace3 = chunk3.eq(WHITESPACE_TABLE.rearrange(chunk3Low)).toLong()
            val whitespace = whitespace0 or (whitespace1 shl 16) or (whitespace2 shl 32) or (whitespace3 shl 48)

            val curlified0 = chunk0.or(0x20.toByte())
            val curlified1 = chunk1.or(0x20.toByte())
            val curlified2 = chunk2.or(0x20.toByte())
            val curlified3 = chunk3.or(0x20.toByte())
            val op0 = curlified0.eq(OP_TABLE.rearrange(chunk0Low)).toLong()
            val op1 = curlified1.eq(OP_TABLE.rearrange(chunk1Low)).toLong()
            val op2 = curlified2.eq(OP_TABLE.rearrange(chunk2Low)).toLong()
            val op3 = curlified3.eq(OP_TABLE.rearrange(chunk3Low)).toLong()
            val op = op0 or (op1 shl 16) or (op2 shl 32) or (op3 shl 48)
        ...
        val remainder = remainder(buffer, length, blockIndex)
        val chunk0 = ByteVector.fromArray(SPECIES_128, remainder, 0)
        val chunk1 = ByteVector.fromArray(SPECIES_128, remainder, 16)
        val chunk2 = ByteVector.fromArray(SPECIES_128, remainder, 32)
        val chunk3 = ByteVector.fromArray(SPECIES_128, remainder, 48)
